      ...
      <section className="relative py-20 overflow-hidden">
        ...
        <div className="container max-w-4xl mx-auto px-4 sm:px-6 py-12">
          <div className="flex flex-col items-center text-center">
            ...
            >
              There are plenty of analytics tools out there. Some are bloated,
              some invasive, and some just too complex. Here's how Satsu stacks
              up – no fluff, just facts.
            </motion.p>
          </div>
        </div>
      </section>
  ...
  const renderValue = (value, serviceName) => {
    ...
    if (value === true || value === "Yes") {
      return (
        <Check
          className={`h-5 w-5 ${
            isSatsu ? "text-violet-400" : "text-emerald-400"
          }`}
        />
      );
      ...
      return (
        <X
          className={`h-5 w-5 ${isSatsu ? "text-zinc-500" : "text-red-400"}`}
        />
      );
      ...
      return (
        <AlertCircle
          className={`h-5 w-5 ${
            isSatsu ? "text-violet-400/70" : "text-amber-400"
          }`}
        />
      );
    } else {
      // For text values
      return (
        <span className={isSatsu ? "text-violet-400 font-medium" : ""}>
          {value}
        </span>
      );
    }
      ...
      <Table>
        <TableHeader>
          <TableRow className="hover:bg-transparent border-zinc-800">
            <TableHead className="w-[180px] font-medium text-zinc-100">
              Feature
            </TableHead>
            ...
            >
              ...
              {services.map((service) => (
                <TableCell
                  ...
                  className={`text-center ${
                    service === "Satsu" ? "bg-zinc-900/50" : ""
                  }`}
                >
                  {renderValue(
                    feature.values[service.toLowerCase().replace(" ", "_")],
                    service
                  )}
                </TableCell>
              ))}
    ...
    privacy: [
      {
        ...
        satsu:
          "Fully privacy-first. No cookies, no personal data collection. GDPR and CCPA compliant out of the box.",
        others:
          "Google Analytics requires consent banners and tracks extensive user data. Plausible and Fathom are also privacy-friendly, while Matomo requires manual configuration for privacy.",
      },
      {
        title: "Consent & Cookie Banners",
        satsu:
          "Never needed – Satsu doesn't use cookies or track personal data.",
        others:
          "Required for Google Analytics. Not needed for Plausible and Fathom. Matomo may require them depending on configuration.",
      },
    ],
    performance: [
      {
        ...
        satsu:
          "Extremely lightweight at under 3KB – minimal impact on your site speed.",
        others:
          "Google Analytics is heavy (40-100KB+). Plausible (~1KB) and Fathom (~1.5KB) are also lightweight. Matomo is significantly heavier, especially self-hosted.",
      },
      {
        title: "Real-Time Dashboard",
        satsu: "Instant updates as traffic comes in – no sampling, no delays.",
        others:
          "Google Analytics often delays data and samples reports. Plausible and Fathom offer real-time stats. Matomo's real-time performance depends on hosting configuration.",
      },
    ],
    setup: [
      {
        ...
        others:
          "Google Analytics requires multiple steps and often Tag Manager. Plausible and Fathom use simple scripts. Matomo requires extensive configuration and usually self-hosting.",
      },
      {
        title: "Hosting & Maintenance",
        satsu: "Fully hosted – no server setup, no maintenance headaches.",
        others:
          "Google Analytics is hosted by Google. Plausible and Fathom offer hosted and self-hosted options. Matomo typically requires self-hosting unless you pay for their cloud service.",
      },
    ],
    business: [
      {
        title: "Pricing Model",
        satsu:
          "Currently free during development. Will remain affordable for developers and indie projects. Core features will always be free.",
        others:
          "Google Analytics is free but monetizes your data. Plausible and Fathom are paid only, with pricing that scales by pageviews. Matomo is free if self-hosted, but cloud version is enterprise-priced.",
      },
      {
        title: "Target Audience",
        satsu:
          "Built for developers, indie hackers, and privacy-focused creators who want simple, honest analytics.",
        others:
          "Google Analytics targets marketers and enterprise teams. Plausible and Fathom focus on ethical site owners and bloggers. Matomo serves large organizations with complex compliance needs.",
      },
    ],
  };
    ...
    >
      <div
        className={`font-medium text-base ${
          highlighted ? "text-violet-400" : "text-zinc-300"
        }`}
      >
        {title}
      </div>
      <div className="text-zinc-300 flex-1">{description}</div>
    </div>
  );
}
    ...
    >
      ...
      <div className="relative z-10">
        <h3 className="text-2xl md:text-3xl font-bold text-white mb-6">
          The simple choice for developers
        </h3>

        <div className="space-y-4 text-zinc-300 mb-8">
          <p>
            Satsu keeps things simple. It's fast, clean, and built for
            developers who want actionable insights without violating user
            trust.
          </p>
          <p>
            Where others go for feature lists and complexity, Satsu focuses on
            clarity, privacy, and a no-nonsense developer experience.
          </p>
          <p>
            If you're tired of GA's bloat, Matomo's complexity, or paying
            monthly just to track a few pages – Satsu might be exactly what
